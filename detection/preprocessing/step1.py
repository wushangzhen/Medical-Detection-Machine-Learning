...
import pydicom as dicom# read dicom
import os
import scipy.ndimage # photo processing lib 
import matplotlib.pyplot as plt # import matlab module
...
def load_scan(path):
    slices = [dicom.read_file(path + '/' + s) for s in os.listdir(path)] # read fourth patient's slice 
    slices.sort(key = lambda x: float(x.ImagePositionPatient[2])) # slice sort according to ImagePositionPatient feature(for 3D picture rendering)
...
def binarize_per_slice(image, spacing, intensity_th=-600, sigma=1, area_th=30, eccen_th=0.99, bg_patch_size=10):
    ...
    for i in range(image.shape[0]):
        ...
        properties = measure.regionprops(label) # compute area connected parts
        valid_label = set()
...
if __name__ == '__main__':
    ...
    plt.imshow((m1+m2)[120])
    plt.figure()
    plt.imshow(case_pixels[120])
    plt.show()
